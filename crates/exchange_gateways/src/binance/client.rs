...
impl BinanceFuturesClient {
  ...
  pub async fn get_klines(
    ...
    let klines: Vec<Vec<serde_json::Value>> = self.get_public("/fapi/v1/klines", &params).await?;
...
impl BinanceFuturesClient {
  ...
  fn convert_binance_position(&self, bp: &AccountPosition) -> Position {
    ...
    Position {
      ...
      liquidation_price: Some(Decimal::from_str(&bp.break_even_price).unwrap_or(Decimal::ZERO)),
      opened_at: Utc::now(),
      closed_at: None,
      updated_at: Utc::now(),
    }
