...
impl KrakenFuturesClient {
  ...
  async fn get_signed<T: serde::de::DeserializeOwned>(&self, endpoint: &str) -> ExchangeResult<T> {
  ...
  pub async fn get_tickers(&self, symbols: Option<&[&str]>) -> ExchangeResult<Vec<Ticker>> {
    let response: KrakenResponse<TickerInfo> = if let Some(syms) = symbols {
      ...
      self
        .get_public("/tickers", &[("symbols", &symbols_param)])
        .await?
    } else {
  ...
  pub async fn get_orderbook(&self, symbol: &str) -> ExchangeResult<OrderbookData> {
    let response: KrakenResponse<Orderbook> =
      self.get_public("/orderbook", &[("symbol", symbol)]).await?;
  ...
  pub async fn send_order(&self, request: &NewOrderRequest) -> ExchangeResult<SendStatus> {
    ...
    let response: KrakenResponse<SendOrderResult> = self.post_signed("/sendorder", &params).await?;
...
mod tests {
  ...
  fn test_endpoint_url() {
    ...
    assert_eq!(
      url,
      "https://demo-futures.kraken.com/derivatives/api/v3/accounts"
    );
  }
  ...
  fn test_new_order_request_limit() {
    let request =
      NewOrderRequest::limit("PI_XBTUSD", "sell", Decimal::from(1), Decimal::from(50000));
