...
impl ReconciliationSnapshot {
  ...
  pub fn new(
    ...
    let now = Utc::now();
    let discrepancies =
      Self::find_discrepancies(&calculated_balances, &reported_balances, threshold);
  ...
  fn find_discrepancies(
    ...
    for (asset, &calc_amount) in calculated {
      ...
      if diff > threshold {
        discrepancies.push(Discrepancy::new(asset.clone(), calc_amount, rep_amount));
      }
    }
    ...
    for (asset, &rep_amount) in reported {
      if !calculated.contains_key(asset) && rep_amount.abs() > threshold {
        discrepancies.push(Discrepancy::new(asset.clone(), Decimal::ZERO, rep_amount));
      }
    }
