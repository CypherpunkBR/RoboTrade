...
impl ScheduledTask {
  ...
  pub fn with_interval(name: impl Into<String>, job_type: JobType, interval_seconds: u32) -> Self {
    let now = Utc::now();
    Self {
...
mod tests {
  ...
  #[test]
  fn test_job_retry() {
    let mut job = Job::new(JobType::OrderExecution, serde_json::json!({})).with_max_attempts(3);
  ...
  fn test_scheduled_task() {
    let task = ScheduledTask::with_interval("sync_market_data", JobType::MarketDataSync, 60)
