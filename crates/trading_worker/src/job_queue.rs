...
impl JobQueue {
  ...
  pub async fn enqueue(&self, job: Job) -> Result<Uuid, String> {
    ...
    let _ = self
      .event_tx
      .send(JobQueueEvent::JobAdded { job_id, job_type });
  ...
  pub async fn enqueue_signal(&self, signal: Signal) -> Result<Uuid, String> {
    ...
    let job = Job::new(JobType::ProcessSignal, priority, JobPayload::Signal(signal));
  ...
  pub async fn enqueue_order(
    ...
    let job = Job::new(JobType::ExecuteOrder, priority, JobPayload::Order(order));
  ...
  pub async fn fail(&self, job_id: Uuid, error: String) {
    ...
    if let Some(mut job) = processing.remove(&job_id) {
      if job.can_retry() {
        ...
        let _ = self
          .event_tx
          .send(JobQueueEvent::JobFailed { job_id, error });
      }
    }
  }
...
mod tests {
  ...
  async fn test_priority_ordering() {
    ...
    let critical = Job::new(
      JobType::CheckStopLoss,
