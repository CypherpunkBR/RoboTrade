...
async fn run_sync_engine<S: SyncDataStore>(
  ...
  for adapter in &adapters {
    ...
    for data_type in &config.data_types {
      ...
      let mut state = store
        .get_sync_state(exchange, *data_type)
        .await
      ...
      info!(
        "Syncing {} {} from {:?}",
        exchange, data_type, state.last_timestamp
      );
      ...
      match result {
        Ok(()) => {
          ...
          state.error = None;
          info!(
            "Completed sync for {} {} - {} items",
            exchange, data_type, state.items_synced
          );
        }
        Err(e) => {
    ...
    let result = match state.data_type {
      ...
      SyncDataType::Deposits => {
        let deposits = adapter
          .fetch_deposits(state.last_timestamp, config.end_time, config.batch_size)
          .await?;
      ...
      SyncDataType::Withdrawals => {
        let withdrawals = adapter
          .fetch_withdrawals(state.last_timestamp, config.end_time, config.batch_size)
          .await?;
