...
use robotrade_core::entities::{OrderRequest, OrderSide, Position, PositionSide, Trade};
...
impl RiskManager {
  ...
  pub async fn validate_order(
    ...
    let daily_loss_limit = self
      .config
      .max_daily_loss
      .min(capital * self.config.max_daily_loss_pct / dec!(100));
    ...
    if !current_positions.is_empty() {
      let symbol_exposure: Decimal = current_positions
        .iter()
  ...
  pub async fn record_trade(&self, trade: &Trade) {
    ...
    if daily_pnl < Decimal::ZERO {
      ...
      if loss >= daily_loss_limit {
        ...
        let _ = self
          .event_tx
          .send(RiskEvent::DailyLossLimitReached { loss });
        warn!(
          "Trading desabilitado: limite de perda diária atingido ({} >= {})",
  ...
  pub async fn update_exposure(&self, positions: &[Position]) {
    ...
    state.total_exposure = positions
      .iter()
      .map(|p| p.quantity * p.current_price * Decimal::from(p.leverage))
      .sum();
    ...
    if state.total_exposure
      > self.config.max_total_exposure * self.config.warning_threshold_pct / dec!(100)
    {
...
mod tests {
  ...
  use robotrade_core::entities::{
    ExchangeId, OrderId, OrderType, PositionId, TimeInForce, TradeCloseReason,
  };
