...
impl<S: TaxLotStore> PnLCalculator<S> {
    ...
    pub async fn process_open(
        ...
        info!(
            "Created tax lot {} for {} {} {} @ {}",
            lot.id, quantity, symbol,
            if side == PositionSide::Long { "LONG" } else { "SHORT" },
            price
        );
    ...
    pub async fn process_close(
        ...
        let mut lots = match self.method {
            ...
            CostBasisMethod::LIFO => {
                self.store.get_open_lots_lifo(exchange, &symbol, side).await
            }
        };
        ...
        match self.method {
            ...
            CostBasisMethod::AverageCost => {
                ...
                for lot in lots.iter_mut() {
                    ...
                    let consume_qty = (quantity * lot_proportion).min(lot.remaining_quantity).min(remaining);
                    remaining -= consume_qty;
