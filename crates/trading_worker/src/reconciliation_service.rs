  ...
  ReconciliationService<E, L, S>
{
  ...
  pub async fn reconcile(&self, exchange: Exchange) -> Result<ReconciliationSnapshot, String> {
    ...
    // Compare balances
    let discrepancies = self.compare_balances(&ledger_balances, &exchange_balances);
    ...
    let total_discrepancy = discrepancies.iter().map(|d| d.difference.abs()).sum();
    ...
    if discrepancies.is_empty() {
      info!(
        "Reconciliation complete for {} - all balances match",
        exchange
      );
    } else {
      warn!(
      ...
      match exchange_map.get(*asset) {
        Some(&exchange_balance) => {
          if let Some(discrepancy) =
            self.check_discrepancy(asset, *ledger_balance, exchange_balance)
          {
...
mod tests {
  ...
  fn test_discrepancy_type() {
    assert_eq!(DiscrepancyType::LedgerHigher, DiscrepancyType::LedgerHigher);
    assert_ne!(
      DiscrepancyType::LedgerHigher,
      DiscrepancyType::ExchangeHigher
    );
