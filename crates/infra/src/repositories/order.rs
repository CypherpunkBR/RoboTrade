...
fn parse_order_source(source_type: &str, source_data: Option<&str>) -> Result<OrderSource, String> {
  match source_type {
    "signal" => {
      ...
      let signal_id = Uuid::parse_str(parts[0]).map_err(|e| format!("Invalid signal_id: {}", e))?;
    ...
    "stop_loss" => {
      ...
      let position_id = Uuid::parse_str(data).map_err(|e| format!("Invalid position_id: {}", e))?;
...
mod tests {
  ...
  async fn test_update_status() {
    ...
    repo
      .update_status(&order.id, OrderStatus::Filled, dec!(0.1), Some(dec!(50000)))
      .await
      .unwrap();
