...
pub trait LedgerRepository: Send + Sync {
  ...
  async fn get_balance_summaries(&self, exchange_id: &str)
    -> InfraResult<Vec<AssetBalanceSummary>>;
...
impl LedgerRepository for SqliteLedgerRepository {
  ...
  async fn get_balance(&self, exchange_id: &str, asset: &str) -> InfraResult<Decimal> {
    ...
    match row {
      Some((balance,)) => Decimal::from_str(&balance)
        .map_err(|e| InfraError::Database(format!("Erro ao parsear saldo: {}", e))),
  ...
  async fn get_balance_summaries(
    &self,
    exchange_id: &str,
  ) -> InfraResult<Vec<AssetBalanceSummary>> {
    ...
    let summaries = rows
      .into_iter()
      .map(
        |(asset, balance, entry_count, last_update)| AssetBalanceSummary {
          asset,
  ...
  async fn count(&self, exchange_id: &str) -> InfraResult<u64> {
    let row: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM ledger_entries WHERE exchange_id = ?")
      .bind(exchange_id)
      .fetch_one(&self.pool)
      .await
...
impl TryFrom<LedgerEntryRow> for LedgerEntry {
  ...
  fn try_from(row: LedgerEntryRow) -> Result<Self, Self::Error> {
    ...
    let timestamp = Utc
      ...
      .single()
      .ok_or_else(|| InfraError::Database("timestamp inválido".to_string()))?;
