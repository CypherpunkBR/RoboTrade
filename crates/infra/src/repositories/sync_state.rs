...
mod tests {
    ...
    async fn test_status_updates() {
        ...
        let found = repo.find("binance", SyncDataType::Orders).await.unwrap().unwrap();
    ...
    async fn test_progress_tracking() {
        ...
        repo.update_progress("binance", SyncDataType::Funding, 50, Some(100))
            .await
            .unwrap();

        let found = repo.find("binance", SyncDataType::Funding).await.unwrap().unwrap();
    ...
    async fn test_mark_completed() {
        ...
        repo.mark_completed("binance", SyncDataType::Deposits)
            .await
            .unwrap();

        let found = repo.find("binance", SyncDataType::Deposits).await.unwrap().unwrap();
