...
use robotrade_core::entities::{AcquisitionType, CostBasisLot, CostBasisLotId, CostBasisMethod};
...
pub trait CostBasisRepository: Send + Sync {
  ...
  async fn find_by_asset(&self, exchange_id: &str, asset: &str) -> InfraResult<Vec<CostBasisLot>>;
  ...
  async fn get_average_cost(&self, exchange_id: &str, asset: &str) -> InfraResult<Option<Decimal>>;

  /// Retorna quantidade total em lotes abertos
  async fn get_total_open_quantity(&self, exchange_id: &str, asset: &str) -> InfraResult<Decimal>;

  /// Retorna custo total dos lotes abertos
  async fn get_total_open_cost(&self, exchange_id: &str, asset: &str) -> InfraResult<Decimal>;
...
impl TryFrom<CostBasisLotRow> for CostBasisLot {
  ...
  fn try_from(row: CostBasisLotRow) -> Result<Self, Self::Error> {
    ...
    let acquisition_date = Utc
      .timestamp_opt(row.acquisition_date, 0)
      .single()
      .ok_or_else(|| InfraError::Database("acquisition_date inválido".to_string()))?;
