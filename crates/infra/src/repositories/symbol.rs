...
impl SymbolRepository for SqliteSymbolRepository {
  async fn find_by_id(&self, id: i64) -> InfraResult<Option<Symbol>> {
    let row: Option<SymbolRow> = sqlx::query_as("SELECT * FROM symbols WHERE id = ?")
      .bind(id)
  ...
  async fn find_by_exchange_and_name(
    ...
    let row: Option<SymbolRow> =
      sqlx::query_as("SELECT * FROM symbols WHERE exchange_id = ? AND symbol = ?")
  ...
  async fn find_by_exchange(&self, exchange_id: ExchangeId) -> InfraResult<Vec<Symbol>> {
    let rows: Vec<SymbolRow> =
      sqlx::query_as("SELECT * FROM symbols WHERE exchange_id = ? ORDER BY symbol")
        .bind(exchange_id.to_string())
        .fetch_all(&self.pool)
        .await
        .map_err(|e| InfraError::Database(e.to_string()))?;
  ...
  async fn find_active(&self) -> InfraResult<Vec<Symbol>> {
    let rows: Vec<SymbolRow> =
      sqlx::query_as("SELECT * FROM symbols WHERE status = 'active' ORDER BY exchange_id, symbol")
        .fetch_all(&self.pool)
        .await
        .map_err(|e| InfraError::Database(e.to_string()))?;
  ...
  async fn count_by_exchange(&self, exchange_id: ExchangeId) -> InfraResult<u64> {
    let row: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM symbols WHERE exchange_id = ?")
      .bind(exchange_id.to_string())
      .fetch_one(&self.pool)
      .await
      .map_err(|e| InfraError::Database(e.to_string()))?;
...
mod tests {
  ...
  fn test_parse_exchange_id() {
    assert_eq!(
      parse_exchange_id("binance_futures").unwrap(),
