...
pub async fn get_ledger_summary(
  ...
  if let Some(summary) = state
    .ledger
    .get_summary(ex, &asset, start_time, end_time)
    .await
  {
    Ok(LedgerSummaryDto {
...
pub async fn get_realized_pnl(
  ...
  let pnl_entries = state
    .pnl
    .get_realized_pnl(ex, sym, start_time, end_time)
    .await;
...
pub async fn calculate_unrealized_pnl(
  ...
  let unrealized = state
    .pnl
    .calculate_unrealized(ex, &symbol, pos_side, current_price)
    .await;
...
pub async fn start_sync(
  ...
  let data_types = request
    .data_types
    .map(|types| types.iter().map(|t| parse_sync_data_type(t)).collect());

  state
    .sync
    .start_sync(exchange, data_types, request.start_time, request.end_time)
    .await
...
pub async fn stop_sync(state: State<'_, AppState>) -> CommandResult<()> {
  ...
  state.sync.stop_sync().await.map_err(|e| CommandError {
    code: "SYNC_ERROR".into(),
    message: e,
  })
}
...
pub async fn pause_sync(state: State<'_, AppState>) -> CommandResult<()> {
  ...
  state.sync.pause_sync().await.map_err(|e| CommandError {
    code: "SYNC_ERROR".into(),
    message: e,
  })
}
...
pub async fn resume_sync(state: State<'_, AppState>) -> CommandResult<()> {
  ...
  state.sync.resume_sync().await.map_err(|e| CommandError {
    code: "SYNC_ERROR".into(),
    message: e,
  })
}
