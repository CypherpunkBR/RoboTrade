...
impl LedgerStore for SqliteLedgerAdapter {
    ...
    async fn get_entries(&self, filters: &LedgerFilters) -> Vec<LedgerEntry> {
        ...
        match self.repo.find_with_filters(&exchange_id, &core_filters).await {
            Ok(entries) => entries.into_iter().map(convert_core_to_service_entry).collect(),
            Err(e) => {
...
fn convert_core_to_service_entry(
    entry: robotrade_core::entities::LedgerEntry,
) -> LedgerEntry {
    ...
    LedgerEntry {
        ...
        reference_type: entry.reference_type.map(|r| format!("{:?}", r).to_lowercase()),
...
impl LedgerState {
    ...
    pub async fn get_summary(
        ...
        let inner = self.inner.read().await;
        if let Some(ref service) = inner.service {
            Some(service.get_summary(exchange, asset, start_time, end_time).await)
        } else {
            None
        }
    }
